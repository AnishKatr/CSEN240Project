...
# Stronger but still reasonable augmentation
...
# EarlyStopping is defined but not used in callbacks below.
# If you want it, add it to the callbacks list.
...
def create_xception_model(input_shape, num_classes=3, learning_rate=5e-5):
    inputs = Input(shape=input_shape, name="Input_Layer")
    base_model = Xception(weights="imagenet", input_tensor=inputs, include_top=False)
    base_model.trainable = False  # freeze backbone for this run
    ...
    x = MultiHeadAttention(
        num_heads=8,
        key_dim=64,  # smaller than channels, faster and more stable
...
cnn_model = create_xception_model(
    input_shape, num_classes=3, learning_rate=5e-5
)

# %%
# Train for 50 epochs. Callbacks only use ReduceLROnPlateau to avoid early stop.
history = cnn_model.fit(
    train_gen_new,
    validation_data=valid_gen_new,
    epochs=350,
    callbacks=[reduce_lr],  # add early_stopping here if you want it
    verbose=1,
)

# %%
# PPO-style loss on validation predictions (post hoc analysis)
y_pred = cnn_model.predict(valid_gen_new)
y_true = valid_gen_new.labels


def ppo_loss(y_true_vals, y_pred_vals):
    epsilon = 0.2
    y_true_one_hot = tf.one_hot(
        tf.cast(y_true_vals, tf.int32), depth=tf.shape(y_pred_vals)[-1]
    )
    selected_probs = tf.reduce_sum(y_pred_vals * y_true_one_hot, axis=-1)
    old_selected_probs = tf.reduce_sum(
        tf.stop_gradient(y_pred_vals) * y_true_one_hot, axis=-1
    )
